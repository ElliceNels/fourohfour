...
async function downloadFile(fileUuid, filename, isOwner) {
  ...
  try {
    ...
    for (let btn of buttons) {
      if (
        btn.onclick &&
        btn.onclick.toString().includes(`downloadFile('${fileUuid}'`)
      ) {
        button = btn;
        break;
      }
    ...
    if (isOwner) {
      ...
      if (encryptedFileKey.length > 44) {
        // Encrypted key is longer due to nonce + ciphertext + base64
      ...
      throw new Error(
        'Shared file download is not yet implemented in the web client. Please use the desktop client for shared files.'
      );
    ...
    const encryptedFileBuffer = sodium.from_base64(
      fileData.encrypted_file,
      sodium.base64_variants.ORIGINAL
    );
...
async function deleteFile(fileUuid, filename) {
  ...
  try {
    ...
    for (let btn of buttons) {
      if (
        btn.onclick &&
        btn.onclick.toString().includes(`deleteFile('${fileUuid}'`)
      ) {
        button = btn;
        break;
      }
